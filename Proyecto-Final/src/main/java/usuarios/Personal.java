...
public class Personal implements Initializable {
    ...
    */
    ...
    com.itextpdf.kernel.colors.Color v_header = new DeviceRgb(196, 49, 0);
    ...
    private void createTableAlertas() {
        ...
        tblPersonal.getColumns()
                .addAll(noAlerta, noOrden, TipoAlerta, descripcion);
        try {
            generales = consultaDAO.getAlertasGenerales(personal.getNoUsuario());
            monitoreadas = consultaDAO.getAlertasMonitoreadas(personal.getNoUsuario());
            for (int i = 0; i < generales.size(); i++)
                alertas.add(generales.get(i));
            for (int i = 0; i < monitoreadas.size(); i++)
                alertas.add(monitoreadas.get(i));
            tblPersonal.setItems(alertas);
    ...
    private void createTableConsultas() {
        ...
        try {
            consultas = consultaDAO.getConsultaUsuario(personal.getNoUsuario());
            tblPersonal.setItems(consultas);
        } catch (SQLException e) {
    ...
    private void createTableOrdenes() {
        ...
        try {
            ordenes = consultaDAO.getOrdenUsuario(personal.getNoUsuario());
            tblPersonal.setItems(ordenes);
        } catch (SQLException e) {
            alertMessage("Error", "createTableConsultas", e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    public void createPdfOrdenes(String dest) throws IOException {
        ...
        for(modeloOrden e : consultaDAO.getListOrdenes(personal.getNoUsuario())) {
